            ...
            <table class="min-w-full divide-y divide-slate-200">
                <thead class="bg-slate-50">
                    <tr>
                        ...
                        <th class="px-2 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider" style="min-width: 280px;">Task</th>
                        <th class="px-2 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider whitespace-nowrap">Timing</th>
                    ...
                    @forelse($tasks as $task)
                        <tr class="{{ $task->status === 'completed' ? 'bg-green-50/50' : '' }} hover:bg-slate-50" wire:key="task-{{ $task->id }}">
                            ...
                            <td class="px-2 py-2 text-xs text-slate-500 whitespace-nowrap">
                                ...
                                @elseif($task->days_before_safari !== null)
                                    @if($task->days_before_safari > 0)
                                        {{ $task->days_before_safari }} days before
                                    @elseif($task->days_before_safari == 0)
                                        Safari start
                                    @else
                                        {{ abs($task->days_before_safari) }} days after
                                    @endif
                                @elseif($task->due_date)
                                    Due: {{ $task->due_date->format('n/j') }}
                                @else
                                    -
                                @endif
                            </td>
