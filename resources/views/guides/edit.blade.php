    ...
    <div class="px-4 py-6 sm:px-0">
        <div class="mb-8">
            <div class="flex items-center">
                ...
                <div>
                    <h1 class="text-3xl font-bold text-slate-900">Edit Guide Assignment</h1>
                    <p class="mt-2 text-sm text-slate-600">Update guide assignment details</p>
                </div>
            </div>
        </div>

        <div class="bg-white overflow-hidden shadow rounded-lg">
            <form action="{{ route('guides.update', $guide) }}" method="POST" class="space-y-6 p-6">
                @csrf
                @method('PATCH')
                    ...
                    <div>
                        ...
                        <select name="country" id="country" required
                            ...
                            @foreach($countries as $country)
                                <option value="{{ $country }}" {{ (old('country', $guide->country) === $country) ? 'selected' : '' }}>
                                    {{ $country }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <datalist id="kenya-guides">
                                @foreach($kenyaGuides as $guideName)
                ...
                <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                    ...
                    <div>
                        ...
                        <input type="date" name="date_from" id="date_from" value="{{ old('date_from', $guide->date_from->format('Y-m-d')) }}" required
                               class="w-full px-3 py-2 border border-slate-300 rounded-md shadow-sm focus:outline-none focus:ring-orange-500 focus:border-orange-500">
                        @error('date_from')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="date" name="date_to" id="date_to" value="{{ old('date_to', $guide->date_to->format('Y-m-d')) }}" required
                               class="w-full px-3 py-2 border border-slate-300 rounded-md shadow-sm focus:outline-none focus:ring-orange-500 focus:border-orange-500">
                        @error('date_to')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                ...
                <div>
                    ...
                    <textarea name="notes" id="notes" rows="3"
                              class="w-full px-3 py-2 border border-slate-300 rounded-md shadow-sm focus:outline-none focus:ring-orange-500 focus:border-orange-500"
                              placeholder="Additional notes about this assignment...">{{ old('notes', $guide->notes) }}</textarea>
                    @error('notes')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
